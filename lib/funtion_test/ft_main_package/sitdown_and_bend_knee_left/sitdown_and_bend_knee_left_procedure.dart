...
import 'sitdown_and_bend_knee_test_left_logic.dart';

class SitdownAndBendKneeLeftProcedure implements ProcedureInterface {
  ...
  functionTestRun(PoseModelVo poseModelVo) {
    if (fullSet.first.index == 0) {
      //준비 자세
      bool ready = ft6Logic.readyFTest(poseModelVo);
      if (ready) {
        if (tempReady == false) {
          startDate = DateTime.now();
          tempReady = true;
      ...
      if (fullSet.first.second != -1) {
        ...
        if (sectionModel.index == nextIndex) {
          ...
          try {
            ft6Logic.fTest(poseModelVo, sectionModel.index); //기능평가 로직 진행
          } catch (e) {
            print(e);
          }
...
final ft6procedure = SitdownAndBendKneeLeftProcedure();
